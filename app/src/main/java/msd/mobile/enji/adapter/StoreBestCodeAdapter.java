...
public class StoreBestCodeAdapter extends RecyclerView.Adapter<StoreBestCodeAdapter.ViewHolder>{
    private Context context;
    private List<PROC_MTD_SALES_STORE_BEST_CODE> listData;

    public StoreBestCodeAdapter(Context context) {
        this.context = context;
        listData = new ArrayList<>();
    }

    public void setData(List<PROC_MTD_SALES_STORE_BEST_CODE> listData) {
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        final PROC_MTD_SALES_STORE_BEST_CODE dataModel = listData.get(position);
        ...
        if (PctAch < 100) {
            holder.tvPctAch.setTextColor(Color.RED);
        }else{
            holder.tvPctAch.setTextColor(Color.BLUE);
        }
